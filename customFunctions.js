...
function generateScene(glContext, vertShader, fragShader) {
	...
	let centerScale = vec3.fromValues(size * 2 - 4 - 0.4, size * 2 - 4 - 0.4, size * 2 - 4 - 0.4);
	...
	for (var i = 1; i < size - 1; i++)
	{
		...
		{//glContext, name, parent = null, ambient, diffuse, specular, n, alpha, texture, textureNorm
			...
			let	temp = createCube(glContext, "FaceFront"+ ((i + j * size).toString()), scene.centerObject, ambient, diffuse, specular, n, alpha, position, scale, vertShader, fragShader);
			scene.objects.forward.push(temp);

			position = vec3.fromValues(i, j, 0);
			temp = createCube(glContext, "FaceBack"+ ((i + j * size).toString()), scene.centerObject, ambient, diffuse, specular, n, alpha, position, scale, vertShader, fragShader);
			scene.objects.backward.push(temp);

			position = vec3.fromValues(i, size-1, j);
			temp = createCube(glContext, "FaceUp"+ ((i + j * size).toString()), scene.centerObject, ambient, diffuse, specular, n, alpha, position, scale, vertShader, fragShader);
			scene.objects.up.push(temp);

			position = vec3.fromValues(i, 0, j);
			temp = createCube(glContext, "FaceDown"+ ((i + j * size).toString()), scene.centerObject, ambient, diffuse, specular, n, alpha, position, scale, vertShader, fragShader);
			scene.objects.down.push(temp);

			position = vec3.fromValues(0, i, j);
			temp = createCube(glContext, "FaceLeft"+ ((i + j * size).toString()), scene.centerObject, ambient, diffuse, specular, n, alpha, position, scale, vertShader, fragShader);
			scene.objects.left.push(temp);

			position = vec3.fromValues(size-1, i, j);
			temp = createCube(glContext, "FaceRight"+ ((i + j * size).toString()), scene.centerObject, ambient, diffuse, specular, n, alpha, position, scale, vertShader, fragShader);
